...
window.UPLOADCARE_PUBLIC_KEY = '3ff2e4c7b24492f75d5c';
window.AVIARY_API_KEY = false;
...
(function () {
    ...
    function prepare () {
        $.getScript('https://unpkg.com/diff-dom@2.5.1/diffDOM.js').then(() => {
        ...
        if (window['AVIARY_API_KEY']) $.getScript('https://cdn.jsdelivr.net/npm/aviary-editor@3.1.0/aviary-editor.js').then(() => {
    ...
    function domUserModified () {
        ...
        future = [];
        saveDOM();
        try { saveChangesToLocalStorage() } catch(e){}
    }
        ...
        $(document).ready(() => {
            ...
            try {
                $('[contenteditable]').each(function () {
                    ...
                    setTimeout(function () {
                        $(that).data('html', $(that).html());
                    }, 1);
                });

                $('[contenteditable]').on('focusin', saveDOM).on('focusout keypress', domUserModified);
            } catch(e){}
    ...
    function saveChangesToLocalStorage () {
        ...
        var changes = localStorage.getItem('_wysiwyg_editor_changes');

        try { changes = JSON.parse(changes); } catch(e){}

        if (!changes) changes = [];

        var newChanges = WYSIWYG.past();

        changes = changes.concat(newChanges);

        try { changes = JSON.stringify(changes); } catch(e){}

        localStorage.setItem('_wysiwyg_editor_changes', changes);
    }
    ...
    $(document).ready(() => {
        ...
        $(window).on('hashchange', function() {
            if (!enabled && location.hash === '#editing') enable();
        });
        ...
        $(document).on('keydown', e => {
            ...
            if (e.keyCode === 83 && e.shiftKey && (e.metaKey || e.ctrlKey)) { // CMD+SHIFT+S to publish changes
                console.log('publishing changes..');

                var changes = localStorage.getItem('_wysiwyg_editor_changes');

                $.post('https://diff-apply-proxy-kzzwkhjbbu.now.sh/', {
            ...
            if (e.keyCode === 83 && (e.metaKey || e.ctrlKey)) { // CMD+S to save changes
                e.preventDefault();
                saveChangesToLocalStorage();
                return false;
            }

            if (e.keyCode === 69 && e.shiftKey && (e.metaKey || e.ctrlKey)) { // CMD+SHIFT+E to toggle editor
                enabled ? disable() : enable();
            }
    ...
    if (changes && changes.length && !window.incomingEditorChanges)
        window.incomingEditorChanges = changes;
})();
